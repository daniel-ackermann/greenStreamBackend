...
export async function getLabelsOfItem(itemId: number):Promise<RowDataPacket[]> {
    const sql = "SELECT count(label) as count, label.name, label.color, label.id FROM information_feedback f, label WHERE f.information_id = ? AND f.done = false AND f.label = label.id GROUP BY label ";
